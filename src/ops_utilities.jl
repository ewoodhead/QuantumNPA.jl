...
function operators(itr; by_party::Bool=false)
    if !by_party
        ...
        return add_monomials!(PartyMonomials(), itr)
    end
end
...
function operators(p::Polynomial; by_party::Bool=false)
    if !by_party
        ...
        return add_monomials!(PartyMonomials(), p)
    end
end
...
function operators(x::Number; by_party::Bool=false)
    return !by_party ? Set{Monomial}() : PartyMonomials()
end

function operators(table::PartyMonomials; by_party::Bool=false)
    if !by_party
